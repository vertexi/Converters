// buck with voltage voltage tuning(GPIO 6 low enable)
...
//    {
//      GpioDataRegs.GPASET.bit.GPIO7 = 1;   // GPIO7 is high
//    }
//    if(GpioDataRegs.GPADAT.bit.GPIO6 == 0)
//    {
//      GpioDataRegs.GPACLEAR.bit.GPIO7 = 1;   // GPIO7 is low
//    }
    if (PID_cal == 1)
    {
      PID_cal = 0;
...
void InitGPIO(void)
{
  ...
  GpioCtrlRegs.GPACTRL.bit.QUALPRD0 = 0x6F;  // qualification period
  EDIS;
}
...
void initMyAdc()
{
  ...
  AdcRegs.ADCSOC2CTL.bit.CHSEL  = 3; // offset voltage
...
int32_t pre_storage_adc1(void)
{
  ...
  ADC1[ConversionCount] = AdcResult.ADCRESULT2;
  adc_sum1 += AdcResult.ADCRESULT2;
  if (counter < sample_size)
  {
    counter++;
...
__interrupt void adc1_isr(void)
{
  ...
  adc_value_aver_1 = pre_storage_adc1()/sample_size;  // offset voltage
//  adc_value_aver_2 = pre_storage_adc2()/sample_size;
//  adc_value_aver_0 = AdcResult.ADCRESULT2;
//  adc_value_aver_1 = AdcResult.ADCRESULT2;  // offset voltage
...
void get_adc_values(void)
{
  ...
  if(GpioDataRegs.GPADAT.bit.GPIO6 == 0)
  {
    offset_voltage = (((adc_value_aver_1*3300)>>12)-1650)*0.001;
  } else{
    offset_voltage = 0;
  }
}
...
void get_PI_signal1(float *error_list)
{
  ...
  error_list[0] = (target_dc+offset_voltage) - adc_value0;
